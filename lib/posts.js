...
/** Upgrade Blogger image URLs to full resolution. */
function upgradeBloggerImage(url) {
  if (!url) return '';
  // Upgrade path-based size params (e.g., /s72-c/ -> /s1600/)
  let upgraded = url.replace(/\/(?:s\d+|w\d+-h\d+)[^\/]*\//gi, '/s1600/');
  // Upgrade query-based size params (e.g., =s72-c -> =s1600)
  // Matches =s123, =w123-h123, and optional suffixes until quote, space, or end
  upgraded = upgraded.replace(/=(?:s\d+|w\d+-h\d+)[a-z0-9-]*(?=["'\s>]|$)/gi, '=s1600');
  return upgraded;
}
...
function sanitizeHtml(html, baseUrl = 'https://imrankhanlincoln.blogspot.com') {
  ...
  // Upgrade Blogger image sizes in HTML
  out = upgradeBloggerImage(out);
...
function extractImage(content, thumbnail) {
  ...
  return m && m[1] ? upgradeBloggerImage(m[1]) : '';
}
...
/**
 * Fix in-body image alt text across all posts.
 *
 * Blogger often emits images with no alt attribute, or with alt text that is
 * just a pile of hashtags (e.g. "#সফলজীবন #মানসিকশান্তি ..."). Both are bad
 * for accessibility and SEO. This post-processing pass replaces any such alt
 * with a descriptive fallback derived from the post title.
 *
 * Applied at the data-processing layer so it covers every post on every build
 * (live fetch AND cached fallback), not via per-file edits.
 */
function fixImageAlts(posts) {
  return (posts || []).map((post) => {
    const title = post.title || '';
    const fallback = `${title} - illustration`.trim();
    const content = String(post.content || '');
    const fixed = content.replace(/<img\b[^>]*>/gi, (tag) => {
      // Skip images that already have a meaningful alt.
      const altMatch = tag.match(/\balt\s*=\s*"([^"]*)"/i);
      if (altMatch) {
        const alt = altMatch[1].trim();
        if (alt && !/^#/.test(alt)) return tag; // has real alt text
      }
      // No alt, empty alt, or hashtag-only alt -> inject descriptive fallback.
      if (/\balt\s*=/i.test(tag)) {
        return tag.replace(/\balt\s*=\s*"[^"]*"/i, `alt="${escapeHtml(fallback)}"`);
      }
      return tag.replace(/<img\b/i, `<img alt="${escapeHtml(fallback)}"`);
    });
    return { ...post, content: fixed };
  });
}
