...
function header(activeKey) {
  ...
  <a class="pill-brand" href="/" aria-label="${escapeHtml(SITE.name)} — Home">
    <img src="/assets/images/logo.png" alt="${escapeHtml(SITE.shortName)}" class="navbar-logo" />
  </a>
...
function footer() {
  ...
  <div class="container footer-grid">
    <div class="footer-col footer-brand">
      <img src="/assets/images/logo-footer.png" alt="${escapeHtml(SITE.name)}" class="footer-logo" />
  ...
  <div class="footer-bottom">
    <div class="container footer-bottom-inner">
      ...
      <p><a href="/privacy-policy">Privacy</a> · <a href="/terms-of-service">Terms</a> · <a href="/sitemap.xml">Sitemap</a> · <a href="/robots.txt">Robots</a> · <a href="${SITE.blogSource}" target="_blank" rel="noopener">Original Blog</a> · Developed by <a href="https://artx.techvrs.com/" target="_blank" rel="noopener">Artx</a></p>
    </div>
  </div>
</footer>`;
}
...
function postCard(post, { showExcerpt = true, revealStagger = null } = {}) {
  ...
  const cardClass = revealStagger !== null ? `post-card reveal` : `post-card`;
...
function generateExpertiseInteractive() {
  ...
  const expertiseTabsHTML = ABOUT.expertise.map((e, index) => {
    ...
    let shortDesc = e.description.split('.')[0] + '.';
    if (shortDesc.length > 90) {
      shortDesc = e.description.substring(0, 85) + '...';
    }

    return `<div class="expertise-tab-wrapper reveal" style="--stagger:${index}">
      <button class="expertise-tab${activeClass}" role="tab" aria-selected="${ariaSelected}" aria-controls="panel-${num}" id="tab-${num}" data-index="${index}">
        <span class="expertise-num">${num}</span>
        <div class="expertise-tab-title">
          <h3>${escapeHtml(e.title)}</h3>
          <p class="truncate-text">${escapeHtml(shortDesc)}</p>
        </div>
      </button>
      <div class="expertise-accordion-body${activeClass}" id="accordion-${num}">
        <div class="expertise-accordion-inner">
          <img src="${image}" alt="${escapeHtml(e.title)}" class="expertise-image-mobile" loading="lazy">
          <p class="expertise-panel-subtitle"><strong>${escapeHtml(e.description)}</strong></p>
          ${e.detailedText ? `<p class="expertise-panel-detail">${escapeHtml(e.detailedText)}</p>` : ''}
        </div>
      </div>
    </div>`;
  }).join('\n');

  const expertisePanelsHTML = ABOUT.expertise.map((e, index) => {
    const num = String(index + 1).padStart(2, '0');
    const isFirst = index === 0;
    const activeClass = isFirst ? ' is-active' : '';
    ...
    return `<div class="expertise-panel${activeClass}" role="tabpanel" id="panel-${num}" aria-labelledby="tab-${num}" aria-hidden="${ariaHidden}">
      <div class="expertise-panel-image-wrapper"><img src="${image}" alt="${escapeHtml(e.title)}" class="expertise-image-desktop" loading="lazy"></div>
      <h3>${escapeHtml(e.title)}</h3>
      <p class="expertise-panel-subtitle"><strong>${escapeHtml(e.description)}</strong></p>
      ${e.detailedText ? `<p class="expertise-panel-detail">${escapeHtml(e.detailedText)}</p>` : ''}
    </div>`;
  ...
  return `
    <div class="expertise-interactive" id="expertiseInteractive">
      <div class="expertise-tablist" role="tablist" aria-orientation="vertical">
        ${expertiseTabsHTML}
      </div>
      <div class="expertise-panels-container reveal" style="--stagger:2" aria-live="polite">
        ${expertisePanelsHTML}
      </div>
    </div>
  `;
...
function becSection() {
  return `
<section class="section section-bec" style="background: var(--color-navy); color: #fff;">
  <div class="container">
    <div class="about-section-layout">
      <div class="about-section-content reveal" style="--stagger:0">
        <p class="eyebrow" style="color: var(--color-primary-soft);">${escapeHtml(BEC.heading)}</p>
        <h2 style="color: #fff;">${escapeHtml(BEC.subHeading)}</h2>
        <p style="color: #e8ecf8; margin-bottom: 24px;">${escapeHtml(BEC.description)}</p>
      ...
      <div class="about-section-img-wrapper reveal" style="--stagger:1">
        <img src="${IMAGES.expertiseGroup}" alt="Bangladesh Executive Chamber" class="about-section-img" loading="lazy">
      </div>
    </div>
  </div>
</section>
...
function homePage({ posts, categories, updates }) {
  ...
  const latestHtml = latest.length
    ? `<div class="post-grid">${latest.map((p, i) => postCard(p, { revealStagger: i })).join('\n')}</div>`
    ...
    <div class="bio-intro-grid">
      <div class="reveal" style="--stagger:0">
        <div class="bio-intro-label" style="margin-bottom: 8px;">
          <img src="/assets/images/logo.png" alt="Imran Khan Lincoln" height="36" style="object-fit: contain;" />
        </div>
...
<!-- Connect With Me -->
<section class="section section-connect bg-grid-pattern">
  <div class="container">
    <div class="section-head reveal">
      <div>
...
` : '';
  ...
  const affiliationsHtml = (AFFILIATIONS || []).map((aff, i) => `
    <div class="achieve-card reveal" style="--stagger:${i}; background: var(--color-bg); padding: 24px; border-radius: var(--radius); box-shadow: var(--shadow-sm); border: 1px solid var(--color-border);">
      <h3 style="margin-bottom: 8px; font-size: 1.25rem;">${escapeHtml(aff.company)}</h3>
      <p style="margin: 0 0 16px 0; font-size: 1rem; color: var(--color-muted);">${escapeHtml(aff.role)} <br/> <em>"${escapeHtml(aff.tagline)}"</em></p>
      <ul style="list-style: none; padding: 0; margin: 0; font-size: 0.95rem; line-height: 1.8;">
        ${aff.phones.map(p => `<li style="display: flex; align-items: center; gap: 8px;"><span style="color: var(--color-primary);">${ICONS.phone}</span> <a href="tel:${p.replace(/[^+\d]/g, '')}" style="color: var(--color-text); text-decoration: none;">${escapeHtml(p)}</a></li>`).join('\n')}
        <li style="display: flex; align-items: center; gap: 8px;"><span style="color: var(--color-primary);">${ICONS.mail}</span> <a href="mailto:${escapeHtml(aff.email)}" style="color: var(--color-text); text-decoration: none;">${escapeHtml(aff.email)}</a></li>
        <li style="display: flex; align-items: center; gap: 8px;"><span style="color: var(--color-primary);">${ICONS.globe}</span> <a href="${escapeHtml(aff.website)}" target="_blank" rel="noopener" style="color: var(--color-text); text-decoration: none;">${escapeHtml(aff.website.replace(/^https?:\/\//, ''))}</a></li>
        <li style="display: flex; align-items: center; gap: 8px;"><span style="color: var(--color-primary);">${ICONS.facebook}</span> <a href="${escapeHtml(aff.facebook)}" target="_blank" rel="noopener" style="color: var(--color-text); text-decoration: none;">Facebook Page</a></li>
        <li style="display: flex; align-items: flex-start; gap: 8px; margin-top: 8px;"><span style="color: var(--color-primary); margin-top: 4px;">${ICONS.pin}</span> <span>${escapeHtml(aff.address)}</span></li>
      </ul>
    </div>
  `).join('\n');

  const affiliationsSection = AFFILIATIONS && AFFILIATIONS.length ? `
<section class="section section-affiliations" style="padding-top: 0;">
  <div class="container">
    <div class="section-head reveal">
      <div>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 400px)); gap: 24px; margin-top: 24px;">
      ${affiliationsHtml}
    </div>
  </div>
</section>
...
<section class="section section-about bg-grid-pattern">
  <div class="container about-grid">
    <div class="about-profile">
      ...
      <h2><img src="/assets/images/logo.png" alt="${escapeHtml(SITE.name)}" height="48" style="object-fit: contain; margin-bottom: 8px;" /></h2>
      <p class="about-role">${escapeHtml(SITE.role)} · <a href="/bec">${escapeHtml(SITE.organization)}</a></p>
    ...
    <div class="about-bio">
      ${geoSummary}
      <div class="stepped-corner-wrapper" style="margin-bottom: 2rem;">
        <img
