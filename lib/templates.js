...
const { SITE, IMAGES, ABOUT, CONTACT, BEC, NAV, TESTIMONIALS, PRIVACY, TERMS } = require('./content');
  ...
  <div class="container">
    <div class="bio-intro-grid">
      <div class="reveal" style="--stagger:0">
        <p class="bio-intro-label">Imran Khan <span class="marquee-dot">•</span> Lincoln</p>
...
function aboutPage() {
  ...
  const servicesHtml = (SITE.services || []).map((s, i) => `
    <div class="achieve-card reveal" style="--stagger:${i}; background: var(--color-bg); padding: 24px; border-radius: var(--radius); box-shadow: var(--shadow-sm);">
      <h3 style="margin-bottom: 12px; font-size: 1.25rem;">${escapeHtml(s.title)}</h3>
      <p style="margin: 0; font-size: 1rem; color: var(--color-muted);">${escapeHtml(s.description)}</p>
    </div>
  `).join('\n');
